...
```sh
pika skills             # report: what is installed, what is projected, what has drifted
pika skills install     # write the canonical skills, regenerate every declared projection
pika skills check       # exit 1 if any projection has drifted
```
...
The `pika:source` lines are what make a stale copy distinguishable from a
current one. **Gate 1 recomputes every one of them**, so a projection generated
before its source moved fails `pika check`:

```
skills projection: AGENTS.md (harness codex) cites skill
.agents/skills/project-work/SKILL.md at sha256:4018…, which is now
sha256:9c2f…; regenerate it with `pika skills install`
```

A region edited by hand reads differently on purpose — it names the region
rather than a source, because the fix is to move the change into the skill, not
...
pika skills check                 # a projection generated before its source moved
```
...
| projection targets (`AGENTS.md`, `CLAUDE.md`, …) | Generated regions inside operator-owned files; the region is kernel-owned, the rest is not | Yes |
