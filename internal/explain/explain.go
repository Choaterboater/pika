...
var errorEntries = map[string]Entry{
	...
	"scope_conflict":   {Kind: KindErrorCode, Rationale: "The scope lease contradicts the request: acquire_scope found the path — or a path inside or containing it — already leased, or release_scope named a path this session does not hold. Leases never expire and are never stolen, so a held scope stays held until its holder gives it back.", Remediation: "The refusal names the holder. Wait for it to release the scope and retry, or lease a narrower path that does not overlap. If the refusal names a holder whose process is gone, its lease file under .project/state/locks/ is stale state: \"pika recover\" reports it and \"pika recover --apply\" clears it. That is the operator's decision, never the agent's."},
