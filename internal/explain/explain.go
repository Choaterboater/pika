...
var errorEntries = map[string]Entry{
	...
	"envelope_denied":  {Kind: KindErrorCode, Rationale: "The capability envelope does not grant this operation. Every mutating tool is deny-by-default.", Remediation: "Run \"pika authorize --scope project\" and retry. It works before a contract exists, but derives no exec grants there: a denied exec names the command it needs, so grant it as a whole argv line, e.g. pika authorize --exec \"make test\"."},
