...
var errorEntries = map[string]Entry{
	...
	"scope_conflict":   {Kind: KindErrorCode, Rationale: "The scope lease contradicts the request: acquire_scope found the path — or a path inside or containing it — already leased, or release_scope named a path this session does not hold. A pika run counts as a holder of every path: it takes a lease on the whole repository and commits through the working tree every scope sits in. Leases never expire and are never stolen, so a held scope stays held until its holder gives it back.", Remediation: "The refusal names the holder — a session's scope lease, or the run holding the repository. Wait for it to be released and retry, or lease a narrower path that does not overlap; a run releases the repository when it finishes. If the refusal names a holder whose process is gone, its lease file under .project/state/locks/ or .project/state/run.lock is stale state: \"pika doctor\" reports it, \"pika recover\" says what it would clear and \"pika recover --apply\" clears it. That is the operator's decision, never the agent's."},
